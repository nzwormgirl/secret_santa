...
options(
  gmailr.oauth = TRUE,
  gmailr.scopes = c("gmail.compose", "gmail.readonly")  # Add any other required scopes
)

# Authenticate with the updated scopes
gm_auth()
...
this_year <- SantasLittleHelper(presents %>% 
                                  filter(!guest %in% "Fay"),"guest",
                   conflictCols = c("partner","presents2019","presents2020","presents2021"))
...
map(this_year$guest, function(myGuest){
  ...
  gm_subject("2024 Whitehead Secret Santa") %>%
